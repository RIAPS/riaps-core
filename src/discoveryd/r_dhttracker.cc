...
namespace riaps::discovery {
    ...
    bool check_state(
        ...
        auto stableCount = count_if(node_list.begin(), node_list.end(),
                                    [](const pair<string, tuple<bool, Timeout<std::chrono::minutes>>> &item) {
                                        return get<0>(item.second);
                                    });

        auto unstableCount = count_if(node_list.begin(), node_list.end(),
                                      [](const pair<string, tuple<bool, Timeout<std::chrono::minutes>>> &item) {
                                          return !get<0>(item.second);
                                      });
